...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ВыборкаКлиент.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон("Дата документа не соответствует договору по работам %1",Константы.ВКМ_НоменклатураРаботыСпециалиста.Получить()), ЭтотОбъект, "Договор",
			Договор, Отказ);
		Возврат;
	КонецЕсли;
